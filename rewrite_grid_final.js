...
export function GanttGrid({ project, phases, members, allocations, tasks, onTaskCreate, onTaskUpdate, onTaskDelete, collapsedPhases, setCollapsedPhases }: GanttGridProps) {
  ...
  const phaseVisuals = useMemo(() => {
    let currentX = 0;
    let currentDay = 0;
    return phaseTimeline.map(p => {
       const isCollapsed = collapsedPhases.has(p.id!);
       const w = isCollapsed ? COLLAPSED_WIDTH : p.durationDays * CELL_WIDTH;
       const left = currentX;
       const startDay = currentDay;
       currentX += w;
       currentDay += p.durationDays;
       return { ...p, left, visualWidth: w, isCollapsed, startDay };
    });
  }, [phaseTimeline, collapsedPhases]);

  const totalGridWidth = phaseVisuals.length > 0 ? phaseVisuals[phaseVisuals.length-1].left + phaseVisuals[phaseVisuals.length-1].visualWidth : 0;

  const getTaskCoordinates = (taskStart: Date, durationDays: number) => {
     const offsetDays = differenceInDays(taskStart, startDate);
     const endDays = offsetDays + durationDays;

     let startX = -1;
     let endX = -1;
     
     for (const p of phaseVisuals) {
         const phaseStart = p.startDay;
         const phaseEnd = phaseStart + p.durationDays;
         
         if (startX === -1) {
             if (offsetDays < phaseStart) startX = p.left;
             else if (offsetDays >= phaseStart && offsetDays < phaseEnd) {
                 if (p.isCollapsed) startX = p.left + 2;
                 else startX = p.left + (offsetDays - phaseStart) * CELL_WIDTH;
             }
         }
         
         if (endX === -1) {
             if (endDays <= phaseStart) endX = p.left;
             else if (endDays > phaseStart && endDays <= phaseEnd) {
                 if (p.isCollapsed) endX = p.left + p.visualWidth - 2;
                 else endX = p.left + (endDays - phaseStart) * CELL_WIDTH;
             }
         }
     }
     
     if (startX === -1) startX = totalGridWidth;
     if (endX === -1) endX = totalGridWidth;

     return { left: startX, width: Math.max(0, endX - startX) };
  };

  const getInitials = (name: string) => name.split(' ').map(w => w[0]).join('').toUpperCase().substring(0, 3);
      ...
      >
        <div className="flex flex-col" style={{ width: totalGridWidth + 256 }}>
                ...
                {phaseVisuals.map(pv => {
                  return (
                    ...
                    >
                       ...
                       {pv.isCollapsed ? <ChevronRight size={14} className="mx-auto" /> : <ChevronLeft size={14} />}
                    </div>
                  )
                })}
              </div>
          ...
          {projectMembers.map(member => (
            <div key={member.id} className="flex h-16 border-b border-slate-100 dark:border-slate-800/50 group/row hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors box-border relative">
              ...
              <div className="absolute left-64 top-0 h-full box-border" style={{ width: totalGridWidth }}>
                ...
                {tasks.filter(t => t.memberId === member.id).map(task => {
                  ...
                  const { left, width } = getTaskCoordinates(taskStart, durationDays);
          ...
          <div className="flex h-16 border-b border-slate-100 dark:border-slate-800/50 transition-colors box-border relative">
            <div className="w-64 shrink-0 sticky left-0 z-30 bg-slate-50 dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col p-2 box-border justify-center shadow-[1px_0_2px_rgba(0,0,0,0.02)]">
              {!showAddMember ? (
                <button 
